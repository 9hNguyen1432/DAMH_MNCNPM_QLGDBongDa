...
class Match {
  ...
  async getMatchByDateTime(date, time) {
    ...
    await database.ref("matchs").once("value", (snapshot) => {
      var temp = snapshot.forEach((childSnapshot) => {
        if (
          childSnapshot.val().date.toString() == date &&
          childSnapshot.val().time.toString() == time
        ) {
          match.add(childSnapshot.val());
        }
      });
  ...
  async getDateNotFinish() {
    //lay cac ngay mà có trận đấu đang diễn ra hoặc chưa diễn ra
    let date_ob = new Date();
    let curdate =
    ...
    await database.ref("matchs").once("value", (snapshot) => {
      var temp = snapshot.forEach((childSnapshot) => {
        ...
        if (
          !date.includes(dt) &&
          (childSnapshot.val().status != "isFinished" || dt == curdate)
        ) {
          date.push(dt);
        }
      });
  ...
  async updateScore(match) {
    ...
    club1.win = club1.win + C1[0];
    club1.draw = club1.draw + C1[1];
    club1.lost = club1.lost + C1[2];
    club2.win = club2.win + C2[0];
    club2.draw = club2.draw + C2[1];
    club2.lost = club2.lost + C2[2];
